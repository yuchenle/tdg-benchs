	...
	for (int i = 0; i < number_of_particles; i++) {
		if (i != id) {
			...
			distance_squared =
                        ...
                        force_magnitude =
                             gravitational_constant * 
                             (d_particles[id].mass) * (d_particles[i].mass) / distance_squared;
	...
	int timestep) 
{
	...
	for (int i = 0; i < number_of_particles; i ++)
                #pragma omp task 
		calculate_force_func(time_interval,number_of_particles,this_particle_array, &output_array[i], i);
