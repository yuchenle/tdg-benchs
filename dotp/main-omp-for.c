...
// double run_dotp(double * A, double * B) {
//     double res = 0.0;
// 
...
// 	// #pragma omp taskwait	
// 	//res = dot_product(N, CHUNK_SIZE, A, B);
// }
// 	return res;
// }
...
int main(int argc, char *argv[]) {
    ...
    if (argc != 4) {
        fprintf(stderr, "Usage: %s <vector size in K> <chunk size in K> <number of iterations>\n", argv[0]);
        return 1;
    }
    ...
    printf ("On average, threads recorded %f ms (nthreads = %d)\n", sum/nthreads, nthreads);

    // printf("result = %f, size (K), %lu, chunk (K), %lu, time (ms), %g\n", res, N/1024, CHUNK_SIZE/1024, makespan);
    printf("result = %f, size (K), %lu, chunk (K), %lu, time %g ms\n", res, N/1024, CHUNK_SIZE/1024, wtime);
    return 0;
}
