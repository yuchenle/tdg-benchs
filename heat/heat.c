...
int main( int argc, char *argv[] )
{
    ...
    {
      printf ("matrix size = %d, NB = %d, NB must be smaller or equal to matrix size\n", param.resolution, Num_B);
      exit ( -1);
    }
    ...
    //#pragma omp master
    {
      for (int i=0; i<num_iter; i++)
      {
          ...
          // taskwait for vanilla version only. This doesn't harm original version to exploit inter-iteration parallelism
          #ifndef TDG
          #pragma omp taskwait
          #endif
